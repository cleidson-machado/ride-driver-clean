...
class FinancialHistoryDaoImpl extends FinancialHistoryDao {
  ...
  Future<FinancialHistoryModel?> getFinancialHistoryById(String id) async {
    final rows = await _db.rawQuery(
        'SELECT * FROM financial_history WHERE id = ?', [id]);
    if (rows.isEmpty) return null;
    return FinancialHistoryModel.fromMap(rows.first);
  }

  @override
  Future<void> insertFinancialHistory(
      FinancialHistoryModel model) async {
    await _db.insert('financial_history', model.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.abort);
  }

  @override
  Future<void> updateFinancialHistory(
      FinancialHistoryModel model) async {
    await _db.update('financial_history', model.toMap(),
        where: 'id = ?', whereArgs: [model.id],
        conflictAlgorithm: sqflite.ConflictAlgorithm.abort);
  }

  @override
  Future<void> deleteFinancialHistory(
      FinancialHistoryModel model) async {
    await _db.delete('financial_history',
        where: 'id = ?', whereArgs: [model.id]);
  }
}
...
class FinancialHistoryPlatformDaoImpl extends FinancialHistoryPlatformDao {
  ...
  Future<List<FinancialHistoryPlatformModel>>
      getPlatformsByFinancialHistoryId(String financialHistoryId) async {
    final rows = await _db.rawQuery(
  ...
  Future<FinancialHistoryPlatformModel?> getFinancialHistoryPlatformById(
      String id) async {
    final rows = await _db.rawQuery(
        'SELECT * FROM financial_history_platform WHERE id = ?', [id]);
    if (rows.isEmpty) return null;
  ...
  Future<void> insertFinancialHistoryPlatform(
      FinancialHistoryPlatformModel model) async {
    await _db.insert('financial_history_platform', model.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.abort);
  }
  ...
  Future<void> updateFinancialHistoryPlatform(
      FinancialHistoryPlatformModel model) async {
    await _db.update('financial_history_platform', model.toMap(),
        where: 'id = ?', whereArgs: [model.id],
        conflictAlgorithm: sqflite.ConflictAlgorithm.abort);
  }
  ...
  Future<void> deleteFinancialHistoryPlatform(
      FinancialHistoryPlatformModel model) async {
    await _db.delete('financial_history_platform',
        where: 'id = ?', whereArgs: [model.id]);
  }
}
...
class PlatformDaoImpl extends PlatformDao {
  ...
  Future<void> insertPlatform(PlatformModel model) async {
    await _db.insert('platform', model.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.abort);
  }

  @override
  Future<void> updatePlatform(PlatformModel model) async {
    await _db.update('platform', model.toMap(),
        where: 'id = ?', whereArgs: [model.id],
        conflictAlgorithm: sqflite.ConflictAlgorithm.abort);
  }
...
class ExtraExpensesDaoImpl extends ExtraExpensesDao {
  ...
  Future<ExtraExpensesModel?> getExtraExpenseById(String id) async {
    final rows = await _db.rawQuery(
        'SELECT * FROM extra_expenses WHERE id = ?', [id]);
    if (rows.isEmpty) return null;
  ...
  Future<List<ExtraExpensesModel>> getExpensesByFinancialHistoryId(
      String financialHistoryId) async {
    final rows = await _db.rawQuery(
  ...
  Future<void> insertExtraExpense(ExtraExpensesModel model) async {
    await _db.insert('extra_expenses', model.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.abort);
  }

  @override
  Future<void> updateExtraExpense(ExtraExpensesModel model) async {
    await _db.update('extra_expenses', model.toMap(),
        where: 'id = ?', whereArgs: [model.id],
        conflictAlgorithm: sqflite.ConflictAlgorithm.abort);
  }
  ...
  Future<void> deleteExtraExpense(ExtraExpensesModel model) async {
    await _db.delete('extra_expenses',
        where: 'id = ?', whereArgs: [model.id]);
  }
}
